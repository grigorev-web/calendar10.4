...
function App() {
  ...
  function getInitialState() {
    ...
    let type = urlParams.get("type") ? urlParams.get("type") : "all-events";
    return {
      range: {
        from: from,
        to: to
  ...
  function handleResetClick() {
    ...
    setState((prevState) => ({
      ...
      select: { type: "all-events", period: "all-period" },
      showEvents: 10,
      reverse: false
    }));
    //console.log(state);
  ...
  const selectedType = selectType.find(
    (option) => option.value === state.select.type
  );

  const selectedPeriod = selectPeriod.find(
    (option) => option.value === state.select.period
  );
  ...
  return (
    <div>
      <DayPicker
        ...
        fromMonth={new Date(new Date().setDate(new Date().getDate() - 18000))}
      />
